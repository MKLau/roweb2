...
##### 4. How to deploy a preview of the book for PRs
...
* To deploy to Netlify _and_ get the preview URL, the workflow doesn't use Netlify's GitHub Actions but instead installs Netlify CI, extracts the URL using `jq` and set it as an environment variable that can be used by next steps. I got the idea from [a thread on Netlify forum](https://community.netlify.com/t/deploying-preview-web-hook-via-api/3320/2).
...
* To post the preview URL in the PR, the workflow uses [a GitHub check run](https://developer.github.com/v3/checks/runs/). You can only create and patch them as a GitHub app. Luckily when in a GitHub Actions runner the default token has these permissions! Using say Travis CI instead of GitHub Actions, everything but posting the link to a GitHub check would work I think, but that part, being able to make the preview link so accessible without implementing our own GitHub app, brought me a lot of joy.
